...
import { LotteryType, ApiResponse, PredictionData, DbRecord } from './types.ts';
import { getZodiac } from './lib/zodiac.ts';
import { getWaveColor } from './lib/wave.ts';

// --- 工具函数 ---
// 解析 JSON 预测数据
const parsePrediction = (predStr: string): PredictionData | null => {
  try {
...
const waveToZh = (w: string) => w === 'red' ? '红波' : w === 'blue' ? '蓝波' : '绿波';
const waveToBg = (w: string) => w === 'red' ? 'bg-red-500' : w === 'blue' ? 'bg-blue-500' : 'bg-green-500';
...
const NumberBall: React.FC<{ 
  ...
  highlight?: boolean; 
  dim?: boolean;
}> = ({ num, size = 'md', showZodiac = false, highlight = false, dim = false }) => {
          ...
          {history.map((rec) => (
            <div key={rec.id} className="bg-white rounded-xl p-3 border border-slate-100 shadow-sm flex flex-col gap-2">
              ...
              <div className="flex flex-wrap gap-1.5 justify-center">
                {rec.open_code.split(',').map((n: string, i: number) => (
...
export default function App() {
  const [activeTab, setActiveTab] = useState<LotteryType>(LotteryType.HK);
  const [data, setData] = useState<ApiResponse | null>(null);
  const [loading, setLoading] = useState(false);
  const [showHistory, setShowHistory] = useState(false);

  const fetchLotteryData = async (type: LotteryType) => {
    setLoading(true);
    try {
  ...
  const verifyResult = (record: DbRecord, pred: PredictionData) => {
